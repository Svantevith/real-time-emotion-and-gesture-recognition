...
def export_data(csv_path, capture_label, row):
    with open(csv_path, mode="a", newline="") as f:
        ...
        header = [
            ...
            *np.ravel(
                [
                    [f"{s}_{i}" for s in columns]
                    for i in range(1, len(row) // len(columns) + 1)
                ]
            ),
        ]
        pd.DataFrame(
            data=np.column_stack([capture_label, *row]), columns=header
...
def live_tracking(
    ...
    if record:
        ...
        writer = cv2.VideoWriter(
            avi_path, cv2.VideoWriter_fourcc(*"MJPG"), 10, (frame_width, frame_height)
        )
        ...
        while cam.isOpened():
            ...
            if face_tracking:
                mp_drawing.draw_landmarks(
                    ...
                    landmark_drawing_spec=mp_drawing.DrawingSpec(
                        color=rgb2bgr(0, 255, 0), thickness=1, circle_radius=1
                    ),
                    connection_drawing_spec=mp_drawing.DrawingSpec(
                        color=rgb2bgr(255, 0, 0), thickness=2, circle_radius=1
                    ),
                )
            ...
            if pose_tracking:
                mp_drawing.draw_landmarks(
                    ...
                    landmark_drawing_spec=mp_drawing.DrawingSpec(
                        color=rgb2bgr(255, 0, 255), thickness=2, circle_radius=2
                    ),
                    connection_drawing_spec=mp_drawing.DrawingSpec(
                        color=rgb2bgr(0, 255, 255), thickness=2, circle_radius=2
                    ),
                )
            ...
            if left_hand_landmarks or right_hand_landmarks:
                row = np.hstack(
                    [
                        prepare_landmark_data(landmarks, HAND_LANDMARKS)
                        for landmarks in hand_landmarks
                    ]
                )
            ...
            if len(row) > 0 and predict and estimator_path and pose_landmarks:
                ...
                if confidence > 0.7:
                    text = "{} {:.2f}%".format(y_pred, confidence * 100)
                    cv2.rectangle(
                        img_bgr,
